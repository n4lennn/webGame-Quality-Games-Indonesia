...
export class Vehicle {
  constructor(type, startX, startY) {
    ...
    this.skid = 0; // tambah ini setelah this.shake = 0
  }
  ...
  update(roadLeft, roadRight) {
    ...
    // Kalau sedang oleng — tidak bisa dikontrol, paksa zigzag kiri kanan
    if (this.skid > 0) {
      this.skid--;

      // Frekuensi oleng makin cepat di awal, makin lambat di akhir
      const frequency = 0.18;
      const amplitude = 7 + (this.skid / 60) * 5; // makin lama makin kecil
      this.vx = Math.sin(this.skid * frequency) * amplitude;

      // Tetap maju tapi tidak bisa ngerem/gas
      this.vy -= this.acceleration * 0.4;
      if (this.vy < -maxSpd * 0.7) this.vy = -maxSpd * 0.7;

      // Visual tilt ikuti arah oleng
      this.angle = this.vx * 1.8;
    } else {
      // Kontrol normal
